...
class Deployer
  ...
  def initialize(subscription_id, resource_group, resource_group_location)
    raise ArgumentError.new("Missing template file 'create-infrastructure-and-grant-access.json' in current directory.") unless File.exist?('create-infrastructure-and-grant-access.json')
  ...
  def deploy
    ...
    template = File.read(File.expand_path(File.join(__dir__, 'create-infrastructure-and-grant-access.json')))
