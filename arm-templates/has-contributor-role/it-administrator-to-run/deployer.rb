...
class Deployer
  ...
  def initialize(subscription_id, resource_group, resource_group_location)
    raise ArgumentError.new("Missing template file 'grant-access.json' in current directory.") unless File.exist?('grant-access.json')
  ...
  def deploy
    ...
    template = File.read(File.expand_path(File.join(__dir__, 'grant-access.json')))
